...
// (ChatGPT) — plus OpenRouter's key-minting PKCE flow, and the shared machinery
// they rely on: PKCE generation, RFC 8628 device-code polling, a local callback
...
// plus arbitrary provider-specific extras (e.g. accountId for OpenAI Codex).
// The extras round-trip through storage via custom JSON (un)marshalling so the
// stored object stays flat.
...
// Expired reports whether the access token is at or past its expiry. Tokens
// with no expiry (Expires == 0) are treated as non-expiring.
...
// OnManualCodeInput is nil). Cancellation is carried by the context passed to
// Login.
