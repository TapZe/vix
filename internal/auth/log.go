...
// a failure can be diagnosed after the fact without reproducing it. The trace
// goes to ~/.vix/logs/auth.log (see AuthLogPath), or VIX_AUTH_LOG=/path to
// redirect the file. Secrets (tokens, auth codes, PKCE verifiers, device codes)
// are never written — only their presence and length.
var (
	logMu      sync.Mutex
