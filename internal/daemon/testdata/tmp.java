...
enum TaskPriority {
    ...
    public String display() { return label + "(" + icon() + ")"; }
}
...
record Task(long id, String name, TaskPriority priority, Map<String, Object> payload) {
    ...
    String describe() {
        ...
        return "Task#%d[%s] priority=%s payload={%s}".formatted(id, name, priority.display(), payloadInfo);
    }
}
...
// 2. Sealed interface
sealed interface PoolEvent permits PoolEvent.TaskSubmitted, PoolEvent.TaskCompleted, PoolEvent.Shutdown {
...
interface Loggable {
    String prefix();
    default void log(String message) { System.out.println("[" + prefix() + "] " + message); }
...
class EventProcessor {
    static String describe(PoolEvent event) {
        ...
        else if (event instanceof PoolEvent.TaskCompleted tc && tc.result().success()) return "OK: task " + tc.result().taskId();
        else if (event instanceof PoolEvent.TaskCompleted tc) return "FAIL: task " + tc.result().taskId();
...
class Worker implements Loggable {
    ...
    Worker(int id, TaskHandler handler) { this.id = id; this.handler = handler; }

    @Override public String prefix() { return "Worker-" + id; }
    public int id() { return id; }

    @Tracked("execute") @SuppressWarnings("unchecked")  // 6. Annotations
    public TaskResult execute(Task task) {
        currentTask = task;
        try { log("Processing: " + task.name()); return handler.handle(task); }
        catch (Exception e) { error("Failed on task " + task.id(), e); return TaskResult.fail(task.id(), e); }
        finally { currentTask = null; }
    }

    // 14. Optional<T>
    public Optional<Task> currentTask() { return Optional.ofNullable(currentTask); }
    public String status() { return currentTask().map(t -> "busy with " + t.name()).orElse("idle"); }
}
...
class WorkerPool implements Loggable, AutoCloseable {
    ...
    WorkerPool(int size, WorkerFactory factory, TaskHandler handler) {
        ...
        // 19. Arrays and collections + 11. var
    ...
    @Override
    public String prefix() { return "Pool"; }
    ...
    public List<TaskResult> submitAll(List<? extends Task> tasks) {
        ...
        var sorted = tasks.stream()
                .sorted(Comparator.comparing(Task::priority, Comparator.comparingInt(TaskPriority::level)).reversed()
                        .thenComparing(Task::name))
                .collect(toList());

        log("Submitting " + sorted.size() + " tasks to " + workers.size() + " workers");

        // 12. Lambda expressions and method references + 13. Stream API
        List<Future<TaskResult>> futures = sorted.stream()
                .map(task -> {
        ...
        return futures.stream()
                .map(f -> {
                    try {
                        ...
                        return TaskResult.fail(-1, e instanceof Exception ex ? ex : new RuntimeException(e));
                    }
                })
                .toList();
    }
    ...
    public String summary(List<TaskResult> results) {
        ...
        var outputs = results.stream().filter(TaskResult::success).map(TaskResult::output).collect(joining(", "));
        var totalLen = results.stream().filter(TaskResult::success).map(r -> r.output().length()).reduce(0, Integer::sum);
        return "Results: %d ok, %d failed, outputs=[%s], totalLen=%d".formatted(ok, fail, outputs, totalLen);
    }
...
public class tmp {
    ...
    public static void main(String[] args) {
        ...
        // 10. Enhanced switch expression with yield
        TaskHandler handler = task -> switch (task.priority()) {
            case CRITICAL -> {
                Thread.sleep(50);
        ...
        try (var pool = new WorkerPool(3, Worker::new, handler)) {
            ...
            var tasks = List.of(
                    new Task(1, "fetch-data", TaskPriority.HIGH, Map.of("url", "https://api.example.com")),
            ...
            // 13. Stream + 12. Lambda + method references
            results.stream().filter(TaskResult::success)
                    .map(r -> "  OK: task %d -> %s".formatted(r.taskId(), r.output())).forEach(System.out::println);
            results.stream().filter(r -> !r.success())
                    .map(r -> "  FAIL: task %d -> %s".formatted(r.taskId(), r.error().getMessage())).forEach(System.out::println);
            System.out.println(pool.summary(results));
            // 14. Optional
            Optional<String> firstOutput = results.stream().filter(TaskResult::success).map(TaskResult::output).findFirst();
