...
func (s Status) String() string { return [...]string{"Pending", "Running", "Done", "Failed"}[s] }
...
func (t *Task) MarkFailed(err error)   { t.Status = Failed; t.Err = err }
...
func (wp *WorkerPool) Close()        { close(wp.jobs); wp.wg.Wait(); close(wp.results) }
...
func main() {
	...
	fmt.Println("safe divide by zero:", err)
	fmt.Println(describeValue("hello"))
	fmt.Println(describeValue(99))
	fmt.Println(describeValue(errors.New("oops")))
	if _, err := parseTask(""); err != nil {
		fmt.Println("parse empty:", err)
	}
	fmt.Println("temp channel:", <-withTempChannel())
	...
	scheduler.Flush()
	_ = buildRegistryMap(scheduler.tasks)
