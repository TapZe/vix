...
import java.util.stream.*; // 6. Annotations (custom)
...
/**
 * Priority levels for tasks submitted to the worker pool. Each level has a numeric weight used for
 * scheduling order.
 *
...
class WorkerPool implements Loggable, AutoCloseable {
  ...
  WorkerPool(int size, WorkerFactory factory, TaskHandler handler) {
    this.executor = Executors.newFixedThreadPool(size); // 19. Arrays and collections + 11. var
  ...
  public List<TaskResult> submitAll(List<? extends Task> tasks) {
    ...
    log(
        "Submitting "
            ...
            + " tasks to "
            + workers.size()
            + " workers"); // 12. Lambda expressions and method references + 13. Stream API
...
public class tmp {
  ...
  public static void main(String[] args) {
    System.out.println(BANNER); // 10. Enhanced switch expression with yield
    TaskHandler handler =
        task ->
    ...
    try (var pool = new WorkerPool(3, Worker::new, handler)) {
      ...
      var tasks =
          List.of(
              ...
              new Task(5, "alert", TaskPriority.CRITICAL)); // 11. var
      ...
      results.stream()
          ...
          .forEach(System.out::println);
      System.out.println(pool.summary(results)); // 14. Optional
      Optional<String> firstOutput =
          results.stream().filter(TaskResult::success).map(TaskResult::output).findFirst();
      System.out.println(
          "First output: " + firstOutput.orElse("none")); // 3. Pattern matching instanceof
      PoolEvent event = new PoolEvent.TaskCompleted(results.getFirst());
      System.out.println("Event: " + EventProcessor.describe(event)); // 5. Wildcards (? super)
      var sink = new ArrayList<Object>();
      pool.drainTo(sink, results);
      System.out.println("Drained " + sink.size() + " results"); // 20. Ternary
