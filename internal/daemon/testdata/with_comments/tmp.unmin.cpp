  ...
  explicit WorkerPool(int num_threads = default_thread_count())
    ...
    for (int i = 0; i < num_threads; ++i)
      workers_.emplace_back(
          [this, i]() { worker_loop(i); }); // 11. Lambda captures
  }
  ~WorkerPool() { shutdown(); }
...
int main() {
  using namespace pool;     // 20. Raw string literal
  ...
  log_all(std::cout, "pool", "starting", "with", default_thread_count(),
          "threads"); // 12. Range-based for + 19. Initializer list
  std::vector<int> task_ids = {10, 20, 30, 40, 50,
                               60, 70, 80}; // 7. Smart pointers (unique_ptr)
  ...
  for (const auto &[priority, label] : meta)
    std::cout << "priority " << priority << " = " << label
              << "\n"; // 11. Lambda: by value [id], by ref [&multiplier], init
                       // capture [offset=100]
  int multiplier = 3;
  for (int id : task_ids) {
  ...
  auto [total, avg] =
      std::make_tuple(stats.tasks_completed,
