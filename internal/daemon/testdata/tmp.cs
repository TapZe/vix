...
var pool = new WorkerPool<string>
{
    MaxConcurrency = 4,
    PoolName = "MainPool"
};

pool.OnTaskCompleted += (wid, item) =>
    Console.WriteLine($"[event] {wid} finished {item.Id}");
 ...
 */
enum WorkerState { Idle, Busy, Draining, Stopped }

enum TaskPriority { Low = 0, Normal = 5, High = 10, Critical = 20 }

// --- Records ---
record class WorkItem(string Id, TaskPriority Priority, Func<CancellationToken, Task<object?>> Execute)
{
...
{
    public string Summary => $"done={Completed}, failed={Failed}, avg={AvgLatencyMs.ToString("F2")}ms";
}

// --- Interface with default implementation ---
interface IWorkerPool<T> where T : class
{
...
/// </summary>
class WorkerPool<T> : IWorkerPool<T> where T : class
{
    ...
    public async Task<T?> SubmitAsync(WorkItem item)
    {
        ...
        try
        {
            ...
            // Pattern matching: switch expression with property & relational patterns
...
{
    public static string Describe<T>(this WorkerPool<T> pool) where T : class
    {
    ...
    public static async Task<List<T?>> SubmitBatch<T>(
        ...
        IEnumerable<WorkItem> items) where T : class
    {
...
{
    public static string Classify(WorkerStats stats) => stats switch
    {
